...
func LoadDayHistory(user string, day rsrc.Day, r rsrc.Reader) ([]info.Song, error) {
	...
	inSongs := data.([][]string)
	outSongs := make([]info.Song, len(inSongs))
...
func WriteDayHistory(songs []info.Song, user string, day rsrc.Day, w rsrc.Writer) error {
